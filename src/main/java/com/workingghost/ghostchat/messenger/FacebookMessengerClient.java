package com.workingghost.ghostchat.messenger;
...
public class FacebookMessengerClient {
    ...
    public Callback deserializeCallback(@NonNull String callbackJSON) {
        ...
        try {
            Callback callback = objectMapper.readValue(callbackJSON, Callback.class);
    ...
    public MessageResponse sendSenderAction(@NonNull String pageAccessToken, @NonNull String recipientId, @NonNull String action) throws FacebookMessengerSendException {
        ...
        checkArgument(StringUtils.isNotBlank(action), "action cannot be empty");

        MessageRequest messageRequest = new MessageRequest();
        User recipient = new User();
        recipient.setId(recipientId);
        messageRequest.setRecipient(recipient);
        messageRequest.setSenderAction(action);
    ...
    private MessageResponse sendMessageRequest(@NonNull String pageAccessToken, @NonNull MessageRequest messageRequest) throws FacebookMessengerSendException {
        ...
        try {
            ...
            httpPost.setEntity(new StringEntity(objectMapper.writeValueAsString(messageRequest), ContentType.APPLICATION_JSON));
            response = httpClient.execute(httpPost);

            int statusCode = response.getStatusLine().getStatusCode();
            if (statusCode >= 200 && statusCode < 300) {
                log.debug("Successfully sent message. messageRequest: {}", messageRequest);
                return objectMapper.readValue(response.getEntity().getContent(), MessageResponse.class);
            } else {
